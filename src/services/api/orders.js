...
export default {
    list() {
        return DB.prepare("SELECT o.*,os.name order_status_name, c.name counterparty_name FROM orders o join order_statuses os on o.order_status_id=os.id join counterparties c on o.counterparty_id=c.id").all();
    },
    show(id) {
    ...
    create(model) {
        ...
        ).run([model.order_status_id, model.counterparty_id, model.created_at, model.updated_at, model.timestamp]);
        ...
        return DB.prepare(
            `SELECT otc.*,tc.name
                 FROM orders_tech_cards otc 
                     JOIN tech_card tc on otc.tech_card_id=tc.id 
    ...
    updateTechCards(order_id, tech_cards) {
        ...
        tech_cards.forEach(item => {
            if (item.id) {
                DB.prepare("UPDATE orders_tech_cards SET tech_card_id=?, quantity=? WHERE id=?").run([
                    item.tech_card_id,
                    item.quantity,
                    item.id,
                ]);
            } else {
                DB.prepare("INSERT INTO orders_tech_cards (order_id, tech_card_id, quantity) VALUES (?,?,?)").run([order_id, item.tech_card_id, item.quantity]);
            }
        })
    }
};
