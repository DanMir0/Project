...
export default {
  ...
  show(id) {
    ...
    let products = DB.prepare(
      "SELECT tcp.*, p.name, mu.name AS measuring_unit FROM tech_cards_products tcp JOIN products p on tcp.product_id = p.id JOIN measuring_units mu on p.measuring_unit_id = mu.id WHERE tech_card_id=? ;"
    ).all(id);
